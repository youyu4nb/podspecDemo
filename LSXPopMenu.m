...
@interface
LSXPopMenuCell : UITableViewCell
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        _titLa.font = [UIFont systemFontOfSize:15];
        _titLa.textColor = RGB(37, 42, 45);
        [self.contentView addSubview:_titLa];
        [_titLa mas_makeConstraints:^(MASConstraintMaker *make) {
...
-(void)setModel:(LSXPopMenuModel *)Model{
    ...
    if (nil != Model.imgName && Model.imgName.length >0) {
        _imgView =
        _imgView =[[UIImageView alloc] initWithImage:[UIImage imageNamed:Model.imgName]];
        ...
        [_titLa mas_remakeConstraints:^(MASConstraintMaker *make) {
...
- (void)setHighlighted:(BOOL)highlighted animated:(BOOL)animated {
    ...
    if (highlighted) {
        self.contentView.backgroundColor = RGB(251, 225, 211);
    } else {
...
-(instancetype)initWithTitles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<LSXPopMenuDelegate>)delegate{
    ...
    if(self=[super init]){
        ...
        _separatorColor = RGB(220, 222, 222);
        ...
        _lArrowPosition = self.width * 0.5 - 0.5 * _lArrowWidth;
        ...
        _contentView.separatorColor = _separatorColor;
        _contentView.height -= 2 * _lArrowHeight;
        _contentView.estimatedRowHeight = 45;
...
- (void)dismiss
{
    [UIView animateWithDuration: 0.25 animations:^{
        if ([self.delegate respondsToSelector:@selector(LSXDidCoseLSXPopupMenu:)]) {
            [self.delegate LSXDidCoseLSXPopupMenu:self];
        }
...
+ (instancetype)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth isShowTriangle:(BOOL)isShowTriangle arrowPosition:(CGFloat)arrowPosition delegate:(id<LSXPopMenuDelegate>)delegate {
    LSXPopMenu * popupMenu=[[LSXPopMenu alloc]initWithTitles:titles icons:icons menuWidth:itemWidth delegate:delegate];
    popupMenu.isShowTriangle=isShowTriangle;
    popupMenu->_lArrowPosition = arrowPosition;
    [popupMenu showRelyOnView:view];
    return popupMenu;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        cell = [[LSXPopMenuCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
        cell.titLa.textColor=_textColor;
        cell.titLa.font=[UIFont systemFontOfSize:_fontSize];
        cell.titLa.numberOfLines = 0;
    }
...
- (void)setArrowPointingWhere: (CGPoint)anchorPoint
{
    ...
    self.x = anchorPoint.x - self.width/2;
